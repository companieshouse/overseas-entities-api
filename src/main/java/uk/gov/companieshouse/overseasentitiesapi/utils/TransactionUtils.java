...
public class TransactionUtils {
    ...
    public boolean isTransactionLinkedToOverseasEntitySubmission(Transaction transaction, String overseasEntitySubmissionSelfLink) {
        ...
        return transaction.getResources().entrySet().stream().map(Entry::getValue)
                .filter(resource -> FILING_KIND_OVERSEAS_ENTITY.equals(resource.getKind())
                        || OLD_FILING_KIND_OVERSEAS_ENTITY.equals(resource.getKind()))
                .anyMatch(resource -> overseasEntitySubmissionSelfLink.equals(resource.getLinks().get(LINK_RESOURCE)));
    }
}
